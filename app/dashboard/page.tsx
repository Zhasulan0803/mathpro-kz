    ...
    <main style={{ minHeight: "100vh", background: "#f7f9fa", fontFamily: "Inter, sans-serif" }}>
      ...
      <nav style={{ background: "white", borderBottom: "1px solid #d1d7dc", padding: "0 1rem", height: "64px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <span style={{ fontSize: "0.8rem", color: "#6a6f73" }}>Жасұлан</span>
          <div style={{ width: "34px", height: "34px", borderRadius: "50%", background: "#5624d0", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: "0.8rem" }}>Ж</div>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "1.5rem 1rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", overflowX: "auto", marginBottom: "1.5rem", paddingBottom: "0.5rem" }}>
          {[
            ...
            { icon: "📜", label: "Сертификат", active: false, href: "/dashboard" },
          ].map((item) => (
            <Link key={item.label} href={item.href} style={{ display: "flex", alignItems: "center", gap: "0.375rem", padding: "0.5rem 0.875rem", textDecoration: "none", background: item.active ? "#5624d0" : "white", color: item.active ? "white" : "#1c1d1f", fontWeight: item.active ? 700 : 400, fontSize: "0.8rem", borderRadius: "4px", border: "1px solid #d1d7dc", whiteSpace: "nowrap", flexShrink: 0 }}>
              <span>{item.icon}</span>
          ...
          {[
            ...
            <div key={l.title} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 0", borderBottom: "1px solid #f7f9fa" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                <div style={{ width: "32px", height: "32px", borderRadius: "4px", background: l.done ? "#e0f5e9" : "#f0e6ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.875rem", flexShrink: 0 }}>
                  {l.done ? "✓" : "▶"}
                </div>
                <div>
              ...
              <button style={{ background: "none", border: "1px solid #5624d0", color: "#5624d0", padding: "0.3rem 0.625rem", borderRadius: "4px", fontSize: "0.7rem", fontWeight: 600, cursor: "pointer", flexShrink: 0, marginLeft: "0.5rem" }}>
                {l.done ? "Қарау" : "Бастау"}
              </button>
            </div>
          ))}
        ...
        {/* BOTTOM NAV */}
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "white", borderTop: "1px solid #d1d7dc", display: "flex", justifyContent: "space-around", padding: "0.625rem 0", zIndex: 50 }}>
          {[
            { icon: "🏠", label: "Басты", href: "/" },
            { icon: "🎬", label: "Сабақ", href: "/dashboard" },
            { icon: "📝", label: "Тест", href: "/tests" },
            { icon: "👤", label: "Профиль", href: "/dashboard" },
          ].map((item) => (
            <Link key={item.label} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.2rem", textDecoration: "none", color: "#6a6f73", fontSize: "0.65rem" }}>
